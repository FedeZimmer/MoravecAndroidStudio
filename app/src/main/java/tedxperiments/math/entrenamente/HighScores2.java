...
public class HighScores2 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		thistitle.setText("Tiempos promedio de respuesta por nivel");
		...
		for (int i=0; i<25; i++) {
			int averagetime;
			if (NNC[i]>0)
				averagetime= (int)(TTC[i])/NNC[i];
			else averagetime=0; 
			data[i] = new GraphViewData(i, averagetime);
			};
		...
		GraphView graphView = new BarGraphView(
		    this // context
		    , "Tiempos promedio de respuesta por nivel en milisegundos" // heading
		);
